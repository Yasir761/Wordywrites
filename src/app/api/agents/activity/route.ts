...
const ALL_AGENTS = [
  ...
  { key: "contentpreview", name: "Content Preview Agent", description: "The Content preview agent distills your full blog into punchy, scroll-stopping teaser lines — perfect for promoting on LinkedIn, Medium, newsletters, or email intros. It captures the essence, hooks, and hashtags in a way that entices readers to click", type: "automated" },
...
export async function GET() {
  ...
  // get plan by userId OR email
  const plan = await getUserPlan(userId, email); // update getUserPlan to accept email too
console.log("User plan in /api/agents/activity:", plan);
  const latestBlog = await BlogModel.findOne({ userId }).sort({ createdAt: -1 }).lean() as { createdAt?: Date } | null;
