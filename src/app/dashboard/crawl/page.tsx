  ...
  const handleEnhance = async () => {
    ...
    try {
      const res = await fetch("/api/agents/crawl", {
        ...
        body: JSON.stringify(
          mode === "url" ? { url } : { manualContent, manualTitle }
        ),
      });
...
showToast({
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-sm">
            Import via URL or paste manually — then WordyWrites enhances readability, structure & SEO without losing your tone.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex gap-2">
            {["url", "manual"].map((m) => (
              <button
                ...
                `}
              >
                {m === "url" ? <Link className="w-4 h-4" /> : <FileText className="w-4 h-4" />}
                {m === "url" ? "Paste URL" : "Paste Content"}
              </button>
            ))}
          </div>
          ...
          {mode === "url" ? (
            <input
              placeholder="Paste any blog URL (Medium, DEV, WordPress, RSS, etc.)"
              value={url}
  ...
  <p className="text-xs text-muted-foreground">
    Fetching article → cleaning content → enhancing with AI…
  </p>
)}
...
{isLowQualityCrawl && (
  <div className="rounded-xl border border-yellow-400/40 bg-yellow-50 dark:bg-yellow-900/20 p-4 text-sm text-yellow-800 dark:text-yellow-200">
     We couldn’t extract the full article content from this URL.
    <br />
    The enhanced version may be incomplete or inferred.
    <br />
    <span className="underline cursor-pointer" onClick={() => {
  setMode("manual");
  setUrl("");
}}>
      Paste content manually for best results.
    </span>
  </div>
)}
        {/* Results */}
        {result && (
          <div className="grid md:grid-cols-2 gap-8">
            ...
            ">
             ...
             {isLowQualityCrawl ? (
  <div className="h-[420px] flex items-center justify-center text-sm text-muted-foreground border rounded-lg">
    Original content could not be fully extracted.
  </div>
) : (
  <MonacoEditor
    height="420px"
  ...
  disabled={isLowQualityCrawl}
  className="
    mt-3 px-4 py-2 rounded-lg text-sm transition
